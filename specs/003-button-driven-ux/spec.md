...
After a user completes timezone registration, a persistent reply keyboard appears at the bottom of the chat and remains visible in all subsequent sessions. The keyboard contains two buttons: "📝 New Task" and "📋 My Tasks". The user never has to remember or type a command to access core bot features.
...
**Independent Test**: A newly registered user can start a fresh chat, see the keyboard, and tap each of the two buttons — confirming the bot responds meaningfully to each — without typing a single command.

**Acceptance Scenarios**:

1. **Given** a user has just selected their timezone, **When** registration completes, **Then** the bot sends a welcome message and a persistent reply keyboard with buttons "📝 New Task" and "📋 My Tasks" appears.
...
**Independent Test**: Open the bot, tap "📝 New Task", type any text, and verify a task is created and confirmed — all without typing a command.

**Acceptance Scenarios**:

1. **Given** a registered user in IDLE state, **When** they tap "📝 New Task", **Then** the bot replies "Enter task text:" and the user's conversation state changes to CREATING_TASK.
2. **Given** a user in CREATING_TASK state, **When** they send any non-empty text message, **Then** the bot creates the task, replies with confirmation (e.g., "✅ Task #N created: [text]"), and returns to IDLE state.
...
**Independent Test**: Tap "📋 My Tasks", verify active tasks appear with three action buttons each, tap "Completed" to switch tabs and verify completed tasks appear with one button each.

**Acceptance Scenarios**:

1. **Given** a registered user with at least one active task, **When** they tap "📋 My Tasks", **Then** the bot shows active tasks by default, each with buttons "⏰ Remind", "✅ Complete", "🗑 Delete".
2. **Given** the task list is showing active tasks, **When** the user taps "Completed", **Then** the list updates to show completed tasks, each with only "🗑 Delete".
3. **Given** the task list is showing completed tasks, **When** the user taps "Active ✓", **Then** the list switches back to active tasks.
4. **Given** a registered user with no tasks, **When** they tap "📋 My Tasks", **Then** the bot shows a friendly empty-state message (e.g., "No tasks yet. Create your first task!").
5. **Given** a task list is displayed, **When** the user taps "✅ Complete" on a task, **Then** the task is marked as done and the list refreshes.

---

### User Story 4 — Set Reminder via Inline Calendar (Priority: P2)

A registered user taps "⏰ Remind" on an active task. An inline calendar appears showing the current month. Past dates are disabled. The user navigates months using forward/back arrows and selects a future date. The bot then asks the user to type the reminder time as free text. The bot parses the time, creates the reminder, and confirms.
...
**Independent Test**: Tap "⏰ Remind" on any active task, navigate the calendar to a future date, tap it, type "14:30", and confirm that a reminder is created for that date and time.

**Acceptance Scenarios**:

1. **Given** a user taps "⏰ Remind" on an active task, **When** the callback is processed, **Then** an inline calendar for the current month is displayed; all dates before today are visually disabled and non-tappable.
...
4. **Given** the calendar is displayed, **When** the user taps a valid future date, **Then** the bot stores the date in the conversation context, updates user state to ENTERING_REMINDER_TIME, and asks "Enter time (e.g.: 14:30, 9 утра, 21:00)".
5. **Given** a user in ENTERING_REMINDER_TIME state, **When** they send a valid time string (HH:mm, H:mm, "N утра", "N вечера"), **Then** the reminder is created, the bot confirms "⏰ Reminder set for [date] [time]", and state returns to IDLE.
...
A user taps "🗑 Delete" on a task. The bot shows a confirmation message with the task's text and two inline buttons: "✅ Yes, delete" and "❌ Cancel". Tapping "✅ Yes, delete" permanently removes the task. Tapping "❌ Cancel" dismisses the confirmation and returns to IDLE.
...
**Independent Test**: Tap "🗑 Delete" on any task, verify the confirmation message appears with both buttons, tap "❌ Cancel" and confirm the task is unchanged; repeat and tap "✅ Yes, delete" and confirm the task is removed.

**Acceptance Scenarios**:

1. **Given** a user taps "🗑 Delete" on a task, **When** the callback is processed, **Then** the bot sends: "Delete task? '[text]'" with inline buttons "✅ Yes, delete" and "❌ Cancel", and state changes to CONFIRMING_DELETE with taskId in context.
2. **Given** a user in CONFIRMING_DELETE state, **When** they tap "✅ Yes, delete", **Then** the task is permanently deleted from the system, the bot confirms deletion, and state returns to IDLE.
3. **Given** a user in CONFIRMING_DELETE state, **When** they tap "❌ Cancel", **Then** no data is changed, the bot sends a cancellation message, and state returns to IDLE.
...
During the registration flow, each timezone option button shows the current time in that timezone alongside the city name (e.g., "🕐 Moscow, SPb (now 18:45)"). This helps users quickly verify which timezone is correct for them without external lookup.
...
- What happens when a user in CREATING_TASK or ENTERING_REMINDER_TIME state taps a button from the persistent keyboard? The bot MUST cancel the current flow, notify the user (e.g., "Task creation cancelled."), reset state to IDLE, and immediately start the newly requested flow.
...
- **FR-001**: The system MUST display a persistent reply keyboard with two buttons ("📝 New Task", "📋 My Tasks") to every registered user after successful registration.
...
- **FR-005a**: When a persistent keyboard button is tapped while the user is in any non-IDLE state, the system MUST cancel the active flow, send a cancellation notification (e.g., "Task creation cancelled."), reset state to IDLE, and then immediately begin the newly requested flow.
- **FR-006**: Tapping "📝 New Task" MUST prompt the user for task text and create the task upon receiving it.
- **FR-007**: Tapping "📋 My Tasks" MUST display the user's task list with a tab toggle between active and completed tasks. Switching tabs MUST edit the existing message in-place (update text and inline buttons) — no new message is sent.
- **FR-008**: Each active task in the list MUST show inline action buttons: "⏰ Remind", "✅ Complete", "🗑 Delete".
- **FR-009**: Each completed task in the list MUST show an inline action button: "🗑 Delete".
- **FR-010**: Tapping "⏰ Remind" on an active task MUST open an inline calendar showing only the current and future months; past dates MUST be visually disabled and non-interactive.
...
- **FR-012**: After the user selects a date on the calendar, the system MUST prompt for a time as free text and support the following input formats: HH:mm, H:mm, "N утра" (1–11, maps to 01:00–11:00), "N вечера" (1–11, maps to 13:00–23:00). Input of "12 утра" or "12 вечера" MUST be rejected with the prompt: "For 12 o'clock use 24-hour format: 00:00 (midnight) or 12:00 (noon)" — the state remains ENTERING_REMINDER_TIME.
...
- **FR-014**: Tapping "🗑 Delete" on any task MUST display a confirmation message naming the task and offer "✅ Yes, delete" and "❌ Cancel" inline buttons before any deletion occurs.
- **FR-015**: Confirming deletion MUST permanently remove the task from the system (hard delete — no soft-delete).
...
- **Task**: An existing entity. Updated by this feature to support inline "Complete" and "Delete" actions triggered by button taps rather than commands only.
...
- Q: When a user in CREATING_TASK or ENTERING_REMINDER_TIME taps a persistent keyboard button, what should happen? → A: Cancel the current flow with a notification ("Task creation cancelled."), reset to IDLE, immediately start the new flow.
- Q: When the task list tab is toggled (Active ↔ Completed), should the bot edit the existing message or send a new one? → A: Edit the existing message in-place — update text and inline buttons without sending a new message.
...
- Q: How should "12 утра" / "12 вечера" be handled in free-text time parsing? → A: Reject with a clarification prompt — "For 12 o'clock use 24-hour format: 00:00 (midnight) or 12:00 (noon)"; state stays ENTERING_REMINDER_TIME.
...
- Task lists with more than ~20 items will be handled by truncation with a note ("Showing first 20 tasks…") as a reasonable default; full pagination is out of scope for this feature.
- Hard delete means a SQL `DELETE` — there is no trash/restore capability in this feature.
