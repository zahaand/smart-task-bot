...
- [ ] CHK001 — Is the conflict between FR-003 ("MUST reuse the existing language-selection
...
- [ ] CHK002 — Is the conflict between FR-015 ("CreateTaskRequest MUST be renamed to
...
- [ ] CHK003 — Is the requirement for adding `CONFIRMING_DELETE_ACCOUNT` to the
...
- [ ] CHK004 — Does the plan cover all 22 Functional Requirements (FR-001 through
...
- [ ] CHK005 — Is the plan's addition of a persistent menu re-send after Reset State
...
- [ ] CHK006 — Is the logical error in `handleDeleteConfirm()` resolved? The method map
...
- [ ] CHK007 — Is the language-resolution mechanism for `routeMenuButton()` when the
...
- [ ] CHK008 — Is it documented whether `SETTINGS_TITLE` and `BTN_SETTINGS` having
...
- [ ] CHK009 — Is it explicitly specified that `SettingsCallbackHandler.handleLanguageChange()`
...
- [ ] CHK010 — Is the `RegistrationGuard` interaction with the new Settings button
...
- [ ] CHK011 — Does the plan specify `@Nested` class structure for
...
- [ ] CHK012 — Is a test scenario defined for the stale-state case in Settings — i.e.,
...
- [ ] CHK013 — Is `@ParameterizedTest` usage required in the plan for
...
- [ ] CHK014 — Is it specified whether `TaskCreationTextHandlerTest` exists and requires
...
- [ ] CHK015 — Does the plan specify that `ReminderTimeTextHandlerTest` must be updated
...
- [ ] CHK016 — Is a test for `UserService.deleteUser()` specified anywhere in the plan?
...
- [ ] CHK017 — Does the plan specify which existing tests assert the 2-button
...
- [ ] CHK018 — Is it documented that removing `@Slf4j` from `TimeParserUtils` (as stated
...
- [ ] CHK019 — Does the plan explicitly state that ALL `SettingsCallbackHandler` fields
...
- [ ] CHK020 — Is it specified that the `deleteUser()` method in `UserService` must log
...
- [ ] CHK021 — Does the plan address that `BotConstants` currently has inline comments
...
- [ ] CHK022 — Is SC-001 ("within two taps from any bot state") measurable for all states?
...
- [ ] CHK023 — Is SC-003 ("zero orphaned records remaining in the database") verifiable
...
- [ ] CHK024 — Are requirements defined for the scenario where a Settings callback
...
- [ ] CHK025 — Is the behavior defined for `/cancel` command received while the user is
...
- [ ] CHK026 — Is the Spring Boot BOM management of `commons-lang3` verified for
...
- [ ] CHK027 — Is the assumption that Phase D (code quality) is independent of Phases B
